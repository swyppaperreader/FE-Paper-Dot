...
export default function MyPage() {
  const [activeTab, setActiveTab] = useState<'documents' | 'account' | 'privacy'>('documents');
      ...
      {documents.length === 0 ? (
        <>
          ...
          <div className={styles.emptyStatePrompt}>
            <p className={styles.emptyStatePromptTitle}>읽은 문서가 생기면 이곳에 자동으로 모여요.</p>
            <div className={styles.emptyStatePromptBox}>
              <p className={styles.emptyStatePromptText}>
                업로드 된 파일이 없습니다.<br />
                텍스트 또는 파일을 번역하고 관리해보세요
              </p>
              <button className={styles.emptyStatePromptButton}>지금 시작하기</button>
            </div>
          </div>
        ...
        <>
          ...
          <div className={styles.recentDocumentPrompt}>
            <p className={styles.recentDocumentPromptText}>
              {mockUser.name}님, <strong>[{latestDocument?.name}]</strong>를 이어서 볼까요?
            </p>
          </div>
          ...
          <table className={styles.documentsTable}>
            <thead className={styles.tableHeader}>
              <tr>
                <th className={styles.tableHeaderCell} style={{ width: '40%' }}>파일명</th>
                <th className={styles.tableHeaderCell} style={{ width: '20%' }}>날짜</th>
                <th className={styles.tableHeaderCell} style={{ width: '20%' }}>용량</th>
              </tr>
            </thead>
            <tbody>
              {documents.map((doc) => (
                <tr key={doc.id} className={styles.tableRow}>
                  <td className={styles.tableCell}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      <span className={`${styles.fileBadge} ${doc.type === 'pdf' ? styles.fileBadgePdf : styles.fileBadgeTxt}`}>
        ...
        <div className={styles.accountFormRow}>
          ...
          <div className={styles.accountSocialLoginRight}>
            <Image
              ...
              priority
            />
            <p className={styles.accountSocialLoginText}>카카오톡 연동 로그인</p>
          </div>
        </div>
        ...
        <div className={styles.sidebar}>
          <div className={styles.buttonGroup}>
            ...
            <button
              ...
              className={`${styles.tabButton} ${activeTab === 'documents' ? styles.tabButtonActive : styles.tabButtonInactive}`}
            >
              내 문서함
            </button>
            ...
            <button
              ...
              className={`${styles.tabButton} ${activeTab === 'account' ? styles.tabButtonActive : styles.tabButtonInactive}`}
            >
              계정 정보
            </button>
            <button
              onClick={() => setActiveTab('privacy')}
              className={`${styles.tabButton} ${activeTab === 'privacy' ? styles.tabButtonActive : styles.tabButtonInactive}`}
            >
              개인정보 & 보안
            </button>
          </div>
